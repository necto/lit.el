...
;;; Commentary:
;; Parse the specifications in lit tests and tester output and represent them as property lists.
;; Supports two formats:
;; - the "//"-style comments with issue specifications found in the lit test files
;; - the output generated by tester when it discovers an issue that is not not specified in the file
...
(defconst lit-parse--target-range-src-regex
  "\\([+-][0-9]*\\)?:\\([0-9]+\\) ?\\([+-][0-9]*\\)?:\\([0-9]+\\)"
  "Regex describing the target-range spec in a source file (relative line offsets).")

(defconst lit-parse--regular-spec-src-regex
  (concat "// \\([A-Z]+\\)\\(\\[[^]]+\\]\\)?\\( +\\)" lit-parse--target-range-src-regex)
...
(defconst lit-parse--regular-spec-indices
  '( :keyword 1
     ...
     :end-line-offset 6
     :end-col 7)
  "Indices of the matched regex corresponding to different elements of the regular spec.")

(defconst lit-parse--edit-src-regex
  (concat "// \\(EDIT\\) [^ ]+ " lit-parse--target-range-src-regex)
...
(defconst lit-parse--edit-indices
  '( :keyword 1
     ...
     :end-line-offset 4
     :end-col 5)
  "Indices of the matched regex corresponding to different elements of the EDIT spec.")
...
(defconst lit-parse--tester-loc-spec-regex
  "\\([0-9]+\\):\\([0-9]+\\) \\([0-9]+\\):\\([0-9]+\\)"
  "Regex matching the absolute target-range specification in tester output.")

(defconst lit-parse--tester-primary-regex
  (concat lit-parse--tester-loc-spec-regex " \\([^:]+\\):\\(.*\\)")
  "Regex matching the primary location in tester output.")
(defconst lit-parse--tester-secondary-regex
  (concat "\\(/[^:]+\\):    " lit-parse--tester-loc-spec-regex ":\\(.*\\)")
  "Regex matching any secondary location in tester output.")
(defconst lit-parse--tester-d-dataflows-regex "[0-9]+ data flows:"
  "Regex matching the line summarizing number of dataflows in the issue.")
(defconst lit-parse--tester-dataflow-descr-regex "// DATAFLOW DESCRIPTION:\\(.*\\)$"
  "Regex matching the DATAFLOW DESCRIPTION of an issue in tester output.")
(defconst lit-parse--tester-fix-descr-regex "// FIX \\(.*\\)$"
  "Regex matching the FIX description of an issue in tester output.")
(defconst lit-parse--tester-edit-regex
  (concat "// EDIT " lit-parse--tester-loc-spec-regex " `\\([^`]*\\)`")
  "Regex matching the EDIT specification in teste output.")

(defun lit-parse--primary-spec (spec-line)
  "Parse the lain with tester-output specification of a primary issue location."
...
(defun lit-parse--secondary-spec (spec-line)
  "Parse the lain with tester-output specification of a secondary issue location."
...
(defun lit-parse--split-into-secondary-dataflow-fix-specs (lines)
  "Split the list of lines into three lists: secondaries, dataflows, fixes.
Tester output lists all of these one after another, so the function finds the
splitting points and cuts the list into three."
  (let ((secondary-specs '())
        (dataflow-specs '())
        (fix-specs '())
...
(defun lit-parse--not-dataflow-descr-p (l)
  "Line is not a DATAFLOW DESCRIPTION spec."
  (not (string-match-p lit-parse--tester-dataflow-descr-regex l)))

(defun lit-parse--dataflows (dataflow-specs)
  "Parse the list of lines containing only dataflow specifications."
  (let ((dataflows '())
    ...
    (while dataflow-specs
      (let ((it-is-descr (string-match lit-parse--tester-dataflow-descr-regex (car dataflow-specs))))
        ...
        (let* ((description (match-string 1 (car dataflow-specs)))
               (step-specs (seq-take-while #'lit-parse--not-dataflow-descr-p (cdr dataflow-specs)))
               (steps (mapcar #'lit-parse--secondary-spec step-specs)))
          (setq dataflow-specs (seq-drop-while #'lit-parse--not-dataflow-descr-p (cdr dataflow-specs)))
          (push `( :description ,description
                   :steps ,(mapcar (lambda (step) (plist-get step :secondary)) steps))
                dataflows))))
    (reverse dataflows)))

(defun lit-parse--not-fix-descr-p (l)
  "Line L is not a FIX description spec."
  (not (string-match-p lit-parse--tester-fix-descr-regex l)))

(defun lit-parse--edit-spec (spec-line)
  "Parse a single EDIT specification from tester output."
...
(defun lit-parse--fixes (fix-specs)
  "Parse the lines FIX-SPECS containing only FIX/EDIT specifications."
  (let ((fixes '())
        (fix-specs fix-specs))
    (while fix-specs
      (let ((it-is-descr (string-match lit-parse--tester-fix-descr-regex (car fix-specs))))
        ...
        (let* ((description (match-string 1 (car fix-specs)))
               (edit-specs (seq-take-while #'lit-parse--not-fix-descr-p (cdr fix-specs)))
               (edits (mapcar #'lit-parse--edit-spec edit-specs)))
          (cl-assert (cl-every #'consp edits)) ; All edits are parsed
          (setq fix-specs (seq-drop-while #'lit-parse--not-fix-descr-p (cdr fix-specs)))
          (push `( :description ,description :edits ,edits)
                fixes))))
    (reverse fixes)))

(defun lit-parse--join-multilines (lines)
  "Fore every line in LINES ending with backslash (\), concat with the next one."
  (let ((joined '())
...
(defun lit-parse--chop-string (string separator)
  "Same as (split-string STRING separator) but preserves the separators.
Separators are kept at the ends of each string.
Modifies MATCH DATA."
  (let ((result '())
        (begin-cur-str 0)
        (end-last-match 0))
